        ...
        function() {
            ...
            _createClass(Iti, [ {
                ...
                value: function _processCountryData() {
                    ...
                    // process the CountryIDs map
                    this._processCountryIDs();
                ...
                key: "_addCountryID",
                value: function _addCountryID(iso2, dialCode, priority) {
                    if (!this.CountryIDs.hasOwnProperty(dialCode)) {
                        this.CountryIDs[dialCode] = [];
                    }
                    var index = priority || 0;
                    this.CountryIDs[dialCode][index] = iso2;
                }
            }, {
                ...
                key: "_processCountryIDs",
                value: function _processCountryIDs() {
                    this.CountryIDs = {};
                    for (var i = 0; i < this.countries.length; i++) {
                        var c = this.countries[i];
                        this._addCountryID(c.iso2, c.dialCode, c.priority);
                        // area codes
                        if (c.areaCodes) {
                            for (var j = 0; j < c.areaCodes.length; j++) {
                                // full dial code is country code + dial code
                                this._addCountryID(c.iso2, c.dialCode + c.areaCodes[j]);
                            }
                        }
                    }
                }
                ...
                value: function _processPreferredCountries() {
                    ...
                    for (var i = 0; i < this.options.preferredCountries.length; i++) {
                        var CountryID = this.options.preferredCountries[i].toLowerCase();
                        var countryData = this._getCountryData(CountryID, false, true);
                ...
                value: function _loadAutoCountry() {
                    ...
                    if (window.intlTelInputGlobals.autoCountry) {
                        ...
                        if (typeof this.options.geoIpLookup === "function") {
                            this.options.geoIpLookup(function(CountryID) {
                                window.intlTelInputGlobals.autoCountry = CountryID.toLowerCase();
                ...
                value: function _updateFlagFromNumber(originalNumber) {
                    ...
                    if (dialCode) {
                        ...
                        var CountryIDs = this.CountryIDs[this._getNumeric(dialCode)];
                        var alreadySelected = CountryIDs.indexOf(this.selectedCountryData.iso2) !== -1;
                        ...
                        if (!isRegionlessNanpNumber && (!alreadySelected || isNanpAreaCode)) {
                            // if using onlyCountries option, CountryIDs[0] may be empty, so we must find the first
                            // non-empty index
                    ...
                    if (CountryID !== null) {
                        return this._setFlag(CountryID);
                    }
                    return false;
                }
            }, {
                ...
                key: "_getCountryData",
                value: function _getCountryData(CountryID, ignoreOnlyCountriesOption, allowFail) {
                    var countryList = ignoreOnlyCountriesOption ? allCountries : this.countries;
                    for (var i = 0; i < countryList.length; i++) {
                        if (countryList[i].iso2 === CountryID) {
                    ...
                    throw new Error("No country data for '".concat(CountryID, "'"));
                }
            }, {
                key: "_setFlag",
                value: function _setFlag(CountryID) {
                    var prevCountry = this.selectedCountryData.iso2 ? this.selectedCountryData : {};
                    // do this first as it will throw an error and stop if CountryID is invalid
                    this.selectedCountryData = CountryID ? this._getCountryData(CountryID, false, false) : {};
                    // update the defaultCountry - we only need the iso2 from now on, so just store that
                    if (this.selectedCountryData.iso2) {
                        this.defaultCountry = this.selectedCountryData.iso2;
                    }
                    this.selectedFlagInner.setAttribute("class", "iti-flag ".concat(CountryID));
                    // update the selected country's title attribute
                    var title = CountryID ? "".concat(this.selectedCountryData.name, ": +").concat(this.selectedCountryData.dialCode) : "Unknown";
                    ...
                    if (this.options.allowDropdown) {
                        ...
                        if (CountryID) {
                            var nextItem = this.countryList.querySelector("#iti-item-".concat(CountryID));
                    ...
                    return prevCountry.iso2 !== CountryID;
                }
            }, {
                    ...
                    if (number.charAt(0) === "+") {
                        ...
                        for (var i = 0; i < number.length; i++) {
                            ...
                            if (!isNaN(parseInt(c, 10))) {
                                ...
                                if (this.CountryIDs[numericChars]) {
                ...
                value: function isValidNumber() {
                    ...
                    var CountryID = this.options.nationalMode ? this.selectedCountryData.iso2 : "";
                    return window.intlTelInputUtils ? intlTelInputUtils.isValidNumber(val, CountryID) : null;
                }
            }, {
                key: "setCountry",
                value: function setCountry(originalCountryID) {
                    var CountryID = originalCountryID.toLowerCase();
                    // check if already selected
                    if (!this.selectedFlagInner.classList.contains(CountryID)) {
                        this._setFlag(CountryID);
